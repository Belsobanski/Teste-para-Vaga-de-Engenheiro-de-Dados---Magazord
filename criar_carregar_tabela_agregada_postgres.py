...
# Caminho de saída para os dados agregados
caminho_csv = r"C:/Users/Belchior/Desktop/Teste Magazord/data_lake/Trusted/clientes_agregados.csv"
...
if clientes_df is not None and produtos_df is not None and transacoes_df is not None:
    ...
    # Agora, vamos criar a tabela agregada
    # Confirmando a estrutura da tabela de transações, se 'id_cliente' e 'quantidade' estão corretos
    if 'id_cliente' in transacoes_df.columns and 'quantidade' in transacoes_df.columns and 'preco' in produtos_df.columns:
        ...
        # Salvar o DataFrame agregado em CSV
        diretorio = os.path.dirname(caminho_csv)
        if not os.path.exists(diretorio):
            os.makedirs(diretorio)

        clientes_agregado_df.to_csv(caminho_csv, index=False)

        print(f"[SUCESSO] Dados agregados salvos em: {caminho_csv}")

        # Agora, você pode carregar os dados agregados para o PostgreSQL
        try:
            clientes_agregado_df.to_sql(
                "clientes_agregados",
                engine,
                if_exists="replace",  # Substituir a tabela caso já exista
                index=False,
                dtype={
        ...
        print("[ERRO] Colunas 'id_cliente', 'quantidade' ou 'preco' não foram encontradas. Verifique os dados.")
